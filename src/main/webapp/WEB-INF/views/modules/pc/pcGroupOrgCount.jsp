...
<head>
	...
	<script type="text/javascript">
		...
		function getGrid(id){
			form.action='${ctx}/pc/pcGroup/count?org='+id;
			form.submit();
		}
		function getGroup(id){
			form.action='${ctx}/pc/pcGroup/list?org='+id+'&flag=hide';
			form.submit();
		}
	</script>
<body>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed" >
		...
		<c:forEach items="${list}" var="pcGroup">
			<tr>
				<td><c:if test="${pcGroup.organization.id!='1'}"><a href="#"  onclick="javaScript:getGrid('${pcGroup.organization.id}');">
				<%-- <a href="${ctx}/pc/pcGroup/form?id=${pcGroup.id}"> --%>
					${pcGroup.organization.name}
				</a></c:if>
				<c:if test="${pcGroup.organization.id=='1'}">${pcGroup.organization.name}</c:if>
				</td>
				<td><a href="#"  onclick="javaScript:getGroup('${pcGroup.organization.id}');">
					${pcGroup.groupCount}
				</a></td>
				<td>
